...
*/
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        if (args.length < 1) {
            ...
            StringBuilder builder = new StringBuilder();
            FontConverterGFX gfx = new FontConverterGFX(new Font(FontName, fstyle, Integer.parseInt(FontSize)));
                     
            gfx.printLetterData(builder);
            
            try (BufferedWriter bw = new BufferedWriter(new FileWriter(fileName + ".GFX.h"))) {
    ...
    public void printLetterData(StringBuilder builder) {
        ...
        builder.append(String.format("0x%02X, ", (byte) firstChar));
        builder.append(String.format("0x%02X, ", (byte) (firstChar + endChar - startChar - 1)));
        builder.append(String.format("%d};", (byte) fontMetrics.getHeight()));

    }

    public List<GfxGlyph> produceLetterDataList() {
        ArrayList<GfxGlyph> letterDataList = new ArrayList<>(endChar - startChar);
