...
set _JAVA_OPTIONS=
prompt $S
echo This is a runner for FontConverterGFX.jar 
echo Build from FontConverterGFX.java source on NetBeans IDE JDK8, see the link 
echo Source dependencies: GfxGlyph.java and lombok.jar
...
echo.
echo java -jar FontConverterGFX.jar [name [size [stylex] [startchar endchar] [outfirstchar]]]
echo.
echo  without parameters: prints the available font names list to console
echo.
echo  with parameter(s): creates a C code text file in current directory
...
echo 	startchar endchar: character set to be parsed, default 32 256, note the max chunk length is 255 chars
...
echo on
java -jar "%~dp0dist\FontConverterGFX.jar" > fonts.txt
@echo off
echo.
echo Example 2: generate C code and binary file for Arial Cyrillic
echo on
java -jar "%~dp0dist\FontConverterGFX.jar" "Arial" 10 c
java -jar "%~dp0dist\FontConverterGFX.jar" "Arial" 16 c
java -jar "%~dp0dist\FontConverterGFX.jar" "Arial" 24 c
@echo off
echo.
echo.
pause